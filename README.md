...
- Tier A 등록 통계: 190개(현행 직접조회 대상 186개, 폐기 매핑 4개는 직접조회 차단)
...
Claude Code에서는 두 방식 중 하나를 고르면 됩니다. 가장 단순한 방법은 `pip install`로 `kosis-analysis-mcp` 실행 명령을 설치한 뒤 로컬 MCP 서버로 등록하는 방식입니다.
...
Claude Code 플러그인 방식은 아래 **방법 9**를 보세요. 이 repo에는 `.claude-plugin/`, `.mcp.json`, `skills/` 구성이 포함되어 있습니다.
...
`query_table` 기본값은 `include_raw=false`입니다. 그래서 각 행에는 `period`, `value`, `unit`, `dimensions`, `value_raw`, `missing_reason`처럼 분석에 필요한 정규화 필드만 들어갑니다. KOSIS 원본 응답 필드 전체가 필요할 때만 `include_raw=true`를 넘기세요.
...
- `KOSIS_MCP_VERBOSE_OUTPUT_CONTRACT`: 기본값 `false`. `true`로 설정하면 `mcp_output_contract`에 전체 `llm_rules`와 `failure_markers` 카탈로그를 포함합니다. 기본 응답은 발생한 marker와 guidance 중심으로 압축됩니다.
...
`chain_full_analysis`는 레거시 진입점으로 남아 있지만, 더 이상 회귀·예측·이상치·차트 생성을 자동 실행하지 않습니다. 대신 `plan_query` → `select_table_for_query` → `resolve_concepts` → `query_table` → `check_variable_compatibility` 같은 분석 재료 수집 워크플로우를 반환합니다. 실제 모델링과 시각화는 LLM이 Python/R 등 코드 실행 환경에서 선택합니다.
...
- `.env.example`: 환경변수 예시
- `artifacts/source_docs/nabo_openapi/`: 로컬에 보관하는 NABO/OpenAPI 엑셀·PDF 원본 근거 자료. `artifacts/`는 `.gitignore` 대상이라 배포나 패키지에는 포함되지 않습니다.
